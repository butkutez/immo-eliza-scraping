...
def combine_csv_files():
    ...
    csv_files = ["antwerp_zivile_51.csv", "brabant_wallon_faranges_1_1532.csv", 
                ...
                "limburg_properties.csv", "luxemburg_tim_1_51.csv", "namur_tim_1_51.csv",
    ...
    df_csv_concat = pd.concat([pd.read_csv(file) for file in csv_paths], ignore_index=True)
    return df_csv_concat

# combine and save
os.makedirs("./data/cleaned_data", exist_ok=True)
df = combine_csv_files()
df.to_csv("./data/cleaned_data/all_scraped_data.csv", index=False)

def clean_csv_file():

    """Remove rows containing the backspace character."""

    df = pd.read_csv("./data/cleaned_data/all_scraped_data.csv")
    pattern = r'\x08'
    ...
    # filtered_df.to_csv("./data/cleaned_data/final_cleaned_data.csv", index=False)
    # return filtered_df
    filtered_df = df[~df.apply(
    lambda row: row.map(lambda x: bool(re.search(pattern, str(x)))).any(), axis=1)]
    filtered_df.to_csv("./data/cleaned_data/final_cleaned_data.csv", index=False)
    return filtered_df
